...
from os import path
import os
from first_num_predict import predict_first_digit
import numpy as np
import sys
import pickle
...
def get_features(sequence_list):
    ...
    esm_1b = get_esm_1b(sequence_list, feature_folder)
    return funcd, esm_1b


def check_length(sequence):
    length = len(sequence)
    if length > 1250 or length < 50:
        error_infor = '''
...
def save_first_digit_result(sequence_list, result_list, output_folder, index):
    result_output_folder = output_folder +'/'+str(index)
    with open(result_output_folder+'/first_digit_result.txt', 'w') as f:
        for i in range(len(sequence_list)):
            f.write('>'+str(result_list[i])+'\n'+sequence_list[i]+'\n')


def save_first_digit_result_single(sequence, result, output_folder, num_id):
    result_output_folder = output_folder +'/'+str(num_id)
    with open(result_output_folder+'/first_digit_result.txt', 'w') as f:
        f.write('>'+str(result)+'\n'+sequence+'\n')


def save_prob_result_single(sequence, result, output_folder, num_id, flag, pred_result):
    result = map(str, result)
    labels = ['0','1','2','3','4','5', '6']
    result_output_folder = output_folder +'/'+str(num_id)
    check_and_mkdir(result_output_folder)
    with open(result_output_folder+'/log.txt', 'w') as f:
        f.write('The {}th sequence: {}\n'.format(num_id,sequence))
...
def main_first(inputfile, outputfile, index):
    ...
    output_folder = saved_feature_folder

    ## load features
    with open(saved_feature_folder+'/funcd_esm1b.pickle', 'rb') as f:
        funcd, esm_1b = pickle.load(f, encoding='iso-8859-1')

    print('We are going to predict whether it is an enzyme and the main class in one step.')

    ## perform predictions in batchs
    first_digit_result, predict_prob = predict_first_digit(funcd, esm_1b, sure_flag_list)
    
    ## test whether it works
    # print("first_digit_result shape")
    # print(len(first_digit_result))
    print("first_digit_result")
    print(first_digit_result)
    ...
    for kk in range(len(first_digit_result)):
        save_prob_result_single(sequence_list[kk], predict_prob[kk], output_folder, kk, sure_flag_list[kk],first_digit_result[kk])
    
    for qq in range(len(first_digit_result)):
        save_first_digit_result_single(sequence_list[qq], first_digit_result[qq], output_folder, qq)
...
if __name__ == "__main__":
    ...
    main_first(inputfile, outputfile, index)
   
   
# code for running the above code:
    # python code_1_pre_processing_first_digit.py -i ../test_sequence/case_study.fasta
    # python code_1_pre_processing_first_digit.py -i ../test_sequence/case_study.fasta -m 0
    # python code_1_pre_processing_first_digit.py -i ../test_sequence/case_study_bk.fasta -m 0
